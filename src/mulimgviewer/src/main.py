...
class MulimgViewer (MulimgViewerGui):
    ...
    def __init__(self, parent, UpdateUI, get_type, default_path=None):
        ...
        #********* 新加：绑定启用视频模式按钮事件 *********
        self.Enable_Video_Mode.Bind(wx.EVT_CHECKBOX, self.on_toggle_video_mode)
        self.is_playing = False  # 当前是否播放状态
        self.timer = wx.Timer(self)  # 定时器控制帧刷新
        self.Bind(wx.EVT_TIMER, self.on_next_frame, self.timer)  # 定时器事件绑定
        self.m_button10.Bind(wx.EVT_BUTTON, self.toggle_play)
        try:
            elf.frame_interval_ms = int(float(self.text_ctrl_interval.GetValue()) * 1000)
        except:
        #self.frame_interval_ms = 100  # 默认 100ms
            self.frame_interval_ms = 100
        #self.slider_img.SetMax(self.video_manager.total_frames - 1)
        # self.slider_img.SetValue(0)  # 重置为起始帧
        # self.slider_value_max.SetLabel(str(self.video_manager.total_frames - 1))  # 显示最大帧数
        # self.slider_img.Bind(wx.EVT_SCROLL_CHANGED, self.on_slider_change)
        #self.slider_img.Bind(wx.EVT_SCROLL_CHANGED, self.on_slider_change)
        self.slider_img.Bind(wx.EVT_SCROLL_CHANGED, self.on_slider_scroll)
        self.slider_img.Bind(wx.EVT_SCROLL_CHANGED, self.on_slider_change)
        self.slider_img.Bind(wx.EVT_SCROLL_THUMBTRACK, self.on_slider_change)  # 确保拖动实时生效

        self.user_seeking = False
        self.slider_value.Bind(wx.EVT_TEXT_ENTER, self.on_slider_value_enter)
        #self.slider_img.Bind(wx.EVT_SCROLL, self.on_slider_change)
        self.slider_img.Bind(wx.EVT_SLIDER, self.on_slider_change)
    ...
    def import_video(self):
        ...
        wildcard = "Video files (*.mp4;*.avi)|*.mp4;*.avi|All files (*.*)|*.*"
        with wx.FileDialog(self, "Choose a video file", wildcard=wildcard,
            ...
            # 显示第一帧
            frame = self.video_manager.get_frame(0)
            ...
            if frame is not None:
                ...
                if isinstance(frame, Image.Image):
                    ...
                    print("⚠️ 非 PIL.Image 类型，跳过显示")
                    return

                # 转换为 wx.Bitmap 显示
                bmp = wx.Bitmap.FromBuffer(frame.width, frame.height, frame.tobytes())
                ...
                self.img_last.SetPosition((10, 10))
                self.img_last.SetSize(bmp.GetSize())
                print("img_panel Size:", self.img_panel.GetSize())
                print("bitmap Size:", bmp.GetSize())
                ...
                self.SetStatusText_(["Import Video", "-1", "Video loaded successfully!", "-1"])
                # 初始化进度条和文本
                self.slider_img.SetMin(0)
                self.slider_img.SetMax(self.video_manager.total_frames - 1)
                self.slider_img.SetValue(0)
                self.slider_value.SetValue("0")
                self.slider_value_max.SetLabel(str(self.video_manager.total_frames - 1))
            else:
                self.SetStatusText_(["Import Video", "-1", "❌ Failed to read frame", "-1"])

    # def show_video_frame(self, pil_img):
    #     import wx
    #     wx_image = wx.Image(pil_img.size[0], pil_img.size[1])
    #     wx_image.SetData(pil_img.convert("RGB").tobytes())
    #     bmp = wx.Bitmap(wx_image)
    ...
    #     self.img_panel.SetSize(wx.Size(pil_img.size[0]+100, pil_img.size[1]+100))
    #     self.img_panel.Layout()
    #     self.img_panel.Refresh()
    # def jump_to_frame(self, frame_idx):
    #     print(f"🟢 尝试跳转到帧：{frame_idx}")
    #     if self.video_manager is None:
    #         print("⚠️ 无 video_manager")
    #         return

    #     if 0 <= frame_idx < self.video_manager.total_frames:
    #         self.user_seeking = True  # ✅ 设为手动操作中

    #         frame = self.video_manager.get_frame(frame_idx)
    #         if frame is not None:
    #             self.show_video_frame(frame)

    #             self.slider_img.SetValue(frame_idx)
    #             self.slider_value.SetValue(str(frame_idx))
    #         else:
    #             print("⚠️ 无法获取帧")

    #         self.user_seeking = False  # ✅ 操作结束
    # def jump_to_frame(self, frame_idx):
    #     print(f"🟢 尝试跳转到帧：{frame_idx}")
    #     if self.video_manager is None:
    #         print("⚠️ 无 video_manager")
    ...
    def jump_to_frame(self, frame_idx):
        ...
        if frame is not None:
            ...
            # 更新控件（UI显示的是逻辑帧号）
            interval = getattr(self, "frame_interval", 1)
            ui_frame_idx = frame_idx // interval
            self.slider_img.SetValue(ui_frame_idx)
            self.slider_value.SetValue(str(ui_frame_idx))
        else:
            print("⚠️ 无法获取帧")
    ...
    #     frame_idx = self.slider_img.GetValue()
    #     print(f"🟡 滑块移动到帧：{frame_idx}")
    #     self.jump_to_frame(frame_idx)
    #     self.slider_value.SetValue(str(frame_idx))  # 同步更新文本
    #     frame = self.video_manager.get_frame_at(frame_idx)
    #     if frame:
    #         self.draw_bitmap_on_panel(frame)
    #         self.video_manager.current_frame_index = frame_idx  # 更新索引
    # def on_slider_value_enter(self, event):
    #     val = self.slider_value.GetValue()
    #     if val.isdigit():
    #         self.jump_to_frame(int(val))
    # # def on_slider_change(self, event):
    # #     self.jump_to_frame(self.slider_img.GetValue())        
    # # def on_slider_change(self, event):
    ...
    def on_slider_value_enter(self, event):
        ...
        interval = getattr(self, "frame_interval", 1)
        real_index = value * interval  # 将UI帧编号映射到实际帧号

        if 0 <= real_index < self.video_manager.total_frames:
            self.jump_to_frame(real_index)
        else:
            print(f"⚠️ 输入帧号 {value} 超出范围")
    ...
    def on_toggle_video_mode(self, event):
        is_checked = self.Enable_Video_Mode.IsChecked()
        if is_checked:
            if self.video_manager is None:
                wx.MessageBox("No video loaded. Please load a video first.", "Info", wx.OK | wx.ICON_INFORMATION)
                return
            ...
            if frame is not None:
                ...
                wx.MessageBox("Failed to load first frame", "Error", wx.OK | wx.ICON_ERROR)
    def toggle_play_pause(self, event):
        if not self.is_playing:
            self.is_playing = True
            self.m_button10.SetLabel("⏸")  # 暂停图标
            interval = self.frame_interval_ctrl.GetValue()  # 从输入框获取播放间隔
            try:
                interval_ms = int(float(interval) * 1000)
            except:
                interval_ms = 100  # 默认100ms
            self.timer.Start(interval_ms)
        else:
            self.is_playing = False
            self.m_button10.SetLabel("▶")  # 播放图标
            self.timer.Stop()
    ...
    def toggle_play(self, event):
        ...
        if not hasattr(self, "timer"):
            ...
            self.Bind(wx.EVT_TIMER, self.on_next_frame, self.timer)
            self.frame_interval_ms = 100  # 默认帧间隔100ms

        if self.is_playing:
            self.timer.Stop()
            self.m_button10.SetLabel("▶")
            self.is_playing = False
        else:
    ...
    #     if not self.video_manager or self.user_seeking:
    #         return  # ✅ 正在跳帧则不自动播放

        frame = self.video_manager.get_next_frame()

        if frame is not None:
            self.show_video_frame(frame)

            current = self.video_manager.current_frame_index
            self.slider_img.SetValue(current)
            self.slider_value.SetValue(str(current))
        else:
            self.timer.Stop()
            self.is_playing = False
            self.m_button10.SetLabel("▶")  # 播放完成恢复按钮
    # def on_slider_change(self, event):
    #     frame_idx = self.slider_img.GetValue()
    #     self.jump_to_frame(frame_idx)
    ...
    def on_next_frame(self, event):
        ...
        if frame is not None:
            ...
            current = self.video_manager.current_frame_index
            self.slider_img.SetValue(current)
            wx.CallAfter(self.slider_value.SetValue, str(current))
            
            #self.slider_value.SetValue(str(current))
            self.slider_value.Refresh()
            self.slider_value.Refresh()
        else:
            self.timer.Stop()
            self.is_playing = False
    ...
    def show_video_frame(self, frame):
        ...
        if isinstance(frame, Image.Image):
            ...
            raise ValueError("Expected a PIL.Image.Image frame")

        # 创建 wx.Bitmap
        bmp = wx.Bitmap.FromBuffer(img.width, img.height, img.tobytes())
        ...
        # 添加到 sizer 中
        sizer = self.img_panel.GetSizer()
        if sizer:
            sizer.Add(self.img_last, 0, wx.ALL | wx.ALIGN_CENTER)
            sizer.Layout()
